...
TipTop first requests the list's first position through Android's accessibility API. It then requests backward scrolls with time for each animation, stopping when the app declines or after 80 steps. Apps that do not expose scroll actions may not respond. The app has no internet permission and does not store screen content.
