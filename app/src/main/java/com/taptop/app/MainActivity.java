...
public class MainActivity extends Activity {
    ...
    private LinearLayout tapZoneOptions;
    ...
    @Override public void onCreate(Bundle state) {
        ...
        section("TAP ZONE");
        LinearLayout tapZoneCard = card();
        toggle(tapZoneCard, "Enable tap zone", "Turn off the tap zone to use TapTop only through shortcuts or intents.", "tap_zone_enabled", true);
        tapZoneOptions = column();
        tapZoneCard.addView(tapZoneOptions, new LinearLayout.LayoutParams(-1, -2));
        tapZoneOptions.setVisibility(prefs.getBoolean("tap_zone_enabled", true) ? View.VISIBLE : View.GONE);
        LinearLayout bar = tapZoneOptions;
        divider(bar);
    ...
    private void showShortcutInfo() {
        ...
        addText(body, "Long-press TapTop’s app icon to find Toggle TapTop, Turn on TapTop, Turn off TapTop, and Scroll to top. You can also select them in apps that support app shortcuts, including launchers and gesture apps.", 14, muted, false, 0, 16);
        ...
        copyableInfo(body, "Turn on action", ShortcutActions.ACTION_TURN_ON);
        copyableInfo(body, "Turn off action", ShortcutActions.ACTION_TURN_OFF);
        ...
        addText(body, "Scrolling uses your selected speed and requires TapTop and its accessibility service to be enabled. Toggle switches on/off. Turn on and Turn off set that state; repeating either leaves it unchanged. Turning on requires accessibility to be enabled.", 13, muted, false, 12, 0);
    ...
    private void updateStatus() {
        ...
        statusDetail.setText(prefs.getBoolean("tap_zone_enabled", true)
                ? "Touch the tap zone to scroll to the top."
                : "Use a shortcut or intent to scroll to the top.");
    }
    ...
    private void toggle(LinearLayout parent, String title, String description, String key, boolean initial) {
        ...
        control.setOnCheckedChangeListener((button, checked) -> {
            ...
            if (key.equals("tap_zone_enabled")) {
                if (tapZoneOptions != null) tapZoneOptions.setVisibility(checked ? View.VISIBLE : View.GONE);
                updateStatus();
            }
